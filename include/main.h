...
public:
	...
	bool checkWyrazenie(void) { //funkcja sprawdzajaca czy podane wyrazenie jest poprawne
		...
		while (found_przecinek != std::string::npos) {
			...
			found_przecinek = wyrazenie.find_first_of("aeiou", found_przecinek + 1);
		}
		...
		for (auto i = 0; i < wyrazenie.length(); i++) {
			if (wyrazenie[i] == '*' && (wyrazenie[size_t(i + 1)] >= '0' && wyrazenie[size_t(i + 1)] <= '9') || (wyrazenie[size_t(i - 1)] >= '0' && wyrazenie[size_t(i - 1)] <= '9')) {
				...
				while (j < wyrazenie.length()) {
					...
					if (j==wyrazenie.length()-1) {
						index_liczby[1][1] = j;
					}
				}
				
				string liczba1 = wyrazenie.substr(index_liczby[0][0], index_liczby[0][1]);
				string liczba2 = wyrazenie.substr(index_liczby[1][0], index_liczby[1][1]);
				auto wynik{ 0.0 };
				if (liczba1.find_first_of(".") || liczba2.find_first_of(".")) {
					auto mnozona{ 0.0 };
					auto mnoznik{ 0.0 };
					auto dziesiec{ 10.0 };
				...
				wyrazenie.insert(wyrazenie.begin() + index_liczby[0][0], wynik);
				for (size_t k = 0; k < 2; k++) {
					for (size_t l = 0; l < 2; l++) {
						index_liczby[k][l] += liczba1.length();
					}
				}
				wyrazenie.erase(index_liczby[0][0],index_liczby[1][1]);
			}
			if (wyrazenie[i] == 'x' && (wyrazenie[size_t(i + 1)] >= '0' && wyrazenie[size_t(i + 1)] <= '9') && (wyrazenie[size_t(i - 1)] >= '0' && wyrazenie[size_t(i - 1)] <= '9')) {
				...
				wyrazenie.erase(index_liczby[0][0], index_liczby[0][1]);
				wyrazenie.erase(index_liczby[1][0], index_liczby[1][1]);
			}
		}
		return;
	}
